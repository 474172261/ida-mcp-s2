...
def run_tests(functions:ida_functions.IDAFunctions):
    ...
    if func_addr:
        test_function(
            "lookup_funcs (by address)",
            functions.lookup_funcs,
            [func_addr]
        )
    ...
    if func_addr:
        test_function(
            "get_int",
            functions.get_int,
            [{"addr": func_addr, "type": "u32le"}]
        )
    
    test_function(
            "read_string",
            functions.read_string,
            ['0x1800A21D8']
        )
    ...
    test_function(
        ...
        [{"ea": 0x180077C14, "offset": 0x20, "name": "RequestBufferLength", "type": "int"}]
    )
